...
mod test_bintree {
    ...
    fn test_bintree_new_simple() {
        ...
        assert!(tree
            .split(SliceData::default(), |left| Ok((left, 22)))
            .unwrap());
        ...
        assert!(tree
            .split(prepare_key(0, 1), |left| Ok((left, 33)))
            .unwrap());
        ...
        assert!(!tree
            .split(prepare_key(0, 1), |left| Ok((left, 34)))
            .unwrap());
        assert!(!tree.update(prepare_key(0, 1), |_| Ok(1)).unwrap());
        assert_eq!(tree3, tree.get_data());
        assert!(!tree
            .merge(prepare_key(1, 1), |left, _right| Ok(left))
            .unwrap());
        assert_eq!(tree3, tree.get_data());
        assert!(!tree
            .merge(prepare_key(0, 2), |left, _right| Ok(left))
            .unwrap());
        assert_eq!(tree3, tree.get_data());
        assert!(!tree
            .merge(prepare_key(1, 2), |left, _right| Ok(left))
            .unwrap());
        assert_eq!(tree3, tree.get_data());

        assert!(tree
            .merge(prepare_key(0, 1), |left, _right| Ok(left))
            .unwrap());
        assert_eq!(tree.get(prepare_key(0, 1)).unwrap(), Some(11));
        assert_eq!(tree.get(prepare_key(1, 1)).unwrap(), Some(24));
    ...
    fn test_bintreeaug_new_simple() {
        ...
        assert!(tree
            .split(prepare_key(0, 1), &33, &CurrencyCollection::with_grams(4))
            .unwrap());
        let tree3 = tree.get_data();
        println!("{}", tree3);
        assert_eq!(tree.get(prepare_key(0, 1)).unwrap(), None);
        ...
        assert!(!tree
            .split(prepare_key(0, 1), &34, &CurrencyCollection::with_grams(5))
            .unwrap());
        assert_eq!(tree3, tree.get_data());
        assert_eq!(tree.root_extra(), &CurrencyCollection::with_grams(7));
    }
    ...
    fn test_bintree_find() {
        ...
        assert_eq!(
            tree.find(SliceData::default()).unwrap(),
            Some((SliceData::default(), 1))
        );
        // 0 and 1
        assert!(tree
            .split(SliceData::default(), |left| Ok((left, 2)))
            .unwrap());
        // 00, 01, 1
        ...
        assert_eq!(
            tree.find(prepare_key(0b0011, 4)).unwrap(),
            Some((prepare_key(1, 3), 5))
        );
        assert_eq!(
            tree.find(prepare_key(0b001111, 6)).unwrap(),
            Some((prepare_key(1, 3), 5))
        );
        assert_eq!(
            tree.find(prepare_key(0b000111, 6)).unwrap(),
            Some((prepare_key(0, 3), 1))
        );
    }
}
